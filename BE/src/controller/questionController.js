...
const handlePostQuestionByAdminController = async (req, res) => {
    try {
        ...
        let qImage = req?.files?.questionImage
        let qAudio = req?.files?.questionAudio
        if (qImage) {
            let data = await questionService.postQuestionByAdminService_Audio(quiz_id, description, qImage);
        ...
        if (qAudio) {
            const filePath = `./src/upload/${qAudio.name}`
            fs.writeFile(filePath, qAudio.data, (err) => {
                if (err) {
